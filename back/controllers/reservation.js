...
//1.사장님이 직접 예약하기 취소
exports.reservationOwnerCancel = async (req, res) => {
  try {
    const { postId } = req.params;
    const userId = req.userId;
    const { reservationId } = req.body;;
    const existingReservation = await Reservation.findOne({ where: { postId , id:reservationId} });

    if (existingReservation) {
      //사장님이 작성한 게시글인지 확인
      Post.findOne({where:{id:postId, userId}}).then( async (post) => {
          await Reservation.destroy({ where: { postId, id:reservationId } });
          res.status(200).send('예약하기 상태를 해제했습니다.');
      })         
    } 
  ...
  try {
    ...
    if (existingReservation) {
      ...
      Post.findOne({where:{id:postId, userId}}).then( async (post) => {
        ...
        await Post.update({sale_status:"거래완료" },{where:{id:postId}});
          res.status(200).send('예약하기 상태를 해제했습니다.');
      })         
    } 
...
exports.getMyReservations = async (req, res, next) => {
  ...
  try {
    ...
    const responseDtos = reservations.map(reservation => {
      ...
      const saleStatus=post.sale_status;
...
//new Date(post.sale_end_date) < new Date()  || post.sale_status==="거래완료"
...
exports.ownerProductReservationList = async (req, res, next) => {
  ...
  try {
    ...
    // 데이터 가공
    const responseDtos = postsWithReservations
      .filter((post) => {
        // 조건: `sale_end_date`가 오늘보다 크거나 같고, `sale_status`가 '거래완료'인 경우 제외
        const isSaleComplete =   post.sale_end_date &&new Date(post.sale_end_date) <  new Date();
        const isSoldOut = post.sale_status === "거래완료";

        return !isSaleComplete && !isSoldOut; // 해당 조건을 만족하지 않는 경우만 포함
      })
      .map((post) => {
        // 유저 정보가 없는 예약은 필터링
