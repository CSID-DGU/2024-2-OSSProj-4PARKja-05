...
exports.getPostById = async (req, res, next) => {
  ...
  try {
    ...
    const responseDto = {
      ...
      reservationStatus: post.Reservations?.length > 0, // 예약 상태
    };
...
exports.shopList = async (req, res, next) => {
  try {
    ...
    const validColumns = ['createdAt', 'name', 'updatedAt']; // 허용된 정렬 컬럼
    const validDirections = ['ASC', 'DESC']; // 허용된 정렬 방향
    const order = [
      [
    ...
    // 검색 조건
    const where = {};
    const userWhere = {}; // User 모델에 대한 검색 조건

    if (searchTerm) {
      userWhere.nick = { [Sequelize.Op.like]: `${searchTerm}` }; // 닉네임에 검색어 포함
    }
      ...
      include: [
        {
          ...
          attributes: ['id', 'nick', 'address'],
          required: true,  // User가 없는 게시물은 제외
          where: userWhere, // User 모델의 검색 조건 추가
        },
      ],
    });
    ...
    // 응답 반환
    console.log("상점 검색 응답 반환 :",responseDtos);
...
exports.myShopList = async (req, res, next) => {
  try {
    ...
    //console.log("나의 상점 리스트 요청:", { id, searchTerm, page, size, sort });
    ...
    const posts = await Post.findAll({
      ...
      include: [
        {
          model: User,
          attributes: ['id', 'nick', 'address'], // 필요한 필드만 가져오기
          required: true, // User가 반드시 존재해야 함
          where: userWhere,
        },
      ],
    });
    ...
    //console.log("상점 리스트 응답 데이터:", responseDtos);
