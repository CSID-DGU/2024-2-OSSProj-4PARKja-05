...
//1.사장님 예약 취소
export const reservationOwnerCancel = async (item) => {
  const response = await instance.post(`/api/reservations/ownerCancel/${item.postId}`, {reservationId:item.reservations[0].reservationId});
  return response.data;
};
...
//2.사장님 거래완료
export const transactionCompleted = async (item) => {
  const response = await instance.post(`/api/reservations/transactionCompleted/${item.postId}`, {reservationId:item.reservations[0].reservationId});
  return response.data;
};
