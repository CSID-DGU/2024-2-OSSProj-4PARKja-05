...
import ReserveNoticeComponent from '../components/ReserveNoticeComponent';

function BoardDetail() {
  const currentBoardId = useLocation().pathname.slice(13);
  const { showAlert } = useAlert();
  const [isReserveNotice, setIsReserveNotice] = useState(false);
  ...
  // * 예약하기 버튼 클릭 핸들러
  const onReserve = () => {
    setIsReserveNotice(true);
  };

  const onReserveCancle = () => {          
  ...
  if(data&& data.status){
    if(data.status==="판매중"){
      reserveButtonContent= <ReserveButton onClick={onReserve}>예약하기</ReserveButton>
       ...
       //예약자  동일한 아이디 인지 확인
       reserveButtonContent= <ReserveCancelButton onClick={onReserveCancle}> 예약 취소하기</ReserveCancelButton>
    }
  }


  if(isReserveNotice){
    return  <ReserveNoticeComponent   setIsReserveNotice={setIsReserveNotice}  
      togglReservationMutation={togglReservationMutation} />
   }
